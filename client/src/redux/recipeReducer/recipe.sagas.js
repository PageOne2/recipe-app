...
export function* getMostRecent({ payload }) {
  try {
    const apiUrl = process.env.NODE_ENV === 'production' ? `${process.env.REACT_APP_API_URL}/recipes/mostRecent/${payload}` : `http://localhost:3000/api/recipes/mostRecent/${payload}`;
...
export function* getMostLiked({ payload }) {
  try {
    const apiUrl = process.env.NODE_ENV === 'production' ? `${process.env.REACT_APP_API_URL}/recipes/mostLiked/${payload}` : `http://localhost:3000/api/recipes/mostLiked/${payload}`;
    let data = yield fetch(apiUrl, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
...
export function* getRecipeById({ payload }) {
  try {
    const apiUrl = process.env.NODE_ENV === 'production' ? `${process.env.REACT_APP_API_URL}/recipes/${payload}` : `http://localhost:3000/api/recipes/${payload}`;
    let data = yield fetch(apiUrl, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
